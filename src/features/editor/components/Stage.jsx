...
function Stage({
  ...
  // and handle touch-based 2-finger rotation gesture for selected layers.
  const touchRotationRef = useRef({
    isRotating: false,
    initialAngle: 0,
    initialRotation: 0,
    layerId: null,
  ...
  useEffect(() => {
    ...
    const handleTouchStart = (e) => {
      ...
      if (e.touches.length === 2 && selectedLayerData.effectiveSelectedLayer) {
        const layer = selectedLayerData.effectiveSelectedLayer
    ...
    const handleTouchMove = (e) => {
      const state = touchRotationRef.current
      if (state.isRotating && e.touches.length === 2 && selectedLayerData.effectiveSelectedLayer) {
        e.preventDefault()
        const currentAngle = getTouchAngle(e.touches)
        const deltaAngleRad = currentAngle - state.initialAngle
        const deltaAngleDeg = (deltaAngleRad * 180) / Math.PI
        let newRotation = state.initialRotation + deltaAngleDeg
        newRotation = (Math.round(newRotation * 10) / 10) % 360
        if (newRotation < 0) newRotation += 360

        // Visual feedback immediately
        const pixiObj = selectedLayerData.selectedLayerObject
        if (pixiObj && !pixiObj.destroyed) {
          pixiObj.rotation = (newRotation * Math.PI) / 180
        }

        if (motionCaptureMode?.isActive) {
          if (motionCaptureMode.onPositionUpdate) {
            motionCaptureMode.onPositionUpdate({
    ...
    const handleTouchEnd = (e) => {
      ...
      if (state.isRotating) {
        ...
        if (motionCaptureMode?.isActive && motionCaptureMode.onInteractionEnd) {
          motionCaptureMode.onInteractionEnd(state.layerId)
        }
      }
    }
  ...
  }, [viewport, selectedLayerData, motionCaptureMode, isPlaying, dispatch])
